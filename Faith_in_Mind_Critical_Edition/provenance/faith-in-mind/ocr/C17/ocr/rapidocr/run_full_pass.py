...
def main() -> None:
    ...
    if RUN_SUMMARY.exists():
        ...
        summary = {"engine": "RapidOCR", "witness_id": "C17", "input_dir": str(PAGE_DIR), "pages_total": len(pages), "pages_with_text": 0, "pages_without_text": [], "errors": [], "pages": [], "resumed": False}
    processed = {page["page_id"] for page in summary.get("pages", [])}
    errored = {page["page_id"] for page in summary.get("errors", [])}
    for page in pages:
        page_id = page.stem
        json_path = OUT_DIR / f"{page_id}.json"
        txt_path = OUT_DIR / f"{page_id}.txt"
        if json_path.exists() and txt_path.exists() and page_id in processed and page_id not in errored:
            continue
        try:
            ...
            json_path.write_text(json.dumps({"page_id": page_id, "source_image": page.name, "results": records, "timing": normalize(elapsed)}, ensure_ascii=False, indent=2), encoding="utf-8")
