...
PAGE_DIR = Path(r"C:\woodblocks\Faith_in_Mind_Critical_Edition\provenance\faith-in-mind\ocr\C17\ocr-input-120dpi")
OUT_DIR = BASE_DIR
...
def load_existing_summary() -> dict[str, Any]:
    ...
    return {"engine": "PaddleOCR", "ocr_version": "PP-OCRv4", "lang": "ch", "device": "cpu", "enable_hpi": False, "enable_mkldnn": False, "cpu_threads": 1, "python": "3.12", "status": "running", "page_results": []}

def main() -> None:
    OUT_DIR.mkdir(parents=True, exist_ok=True)
    summary = load_existing_summary()
    summary["resumed"] = RUN_SUMMARY.exists()
    seen = {entry["page"] for entry in summary.get("page_results", []) if entry.get("status") == "success"}
    ocr = PaddleOCR(lang="ch", ocr_version="PP-OCRv4", device="cpu", enable_hpi=False, enable_mkldnn=False, cpu_threads=1)
    pages = sorted(PAGE_DIR.glob("C17-p*.jpg"))
    for page in pages:
        page_id = page.stem
        ...
        try:
            ...
            if obj is None:
                obj = {"repr": repr(item)}
            if isinstance(obj, dict):
                texts = obj.get("rec_texts") or obj.get("rec_text") or obj.get("res", {}).get("rec_texts") or obj.get("res", {}).get("rec_text") or []
                if not isinstance(texts, list):
                    texts = [str(texts)]
            else:
                texts = [repr(item)]
            json_path.write_text(json.dumps(obj, ensure_ascii=False, indent=2), encoding="utf-8")
            txt_path.write_text("\n".join(str(t) for t in texts), encoding="utf-8")
        ...
        summary["page_results"] = [entry for entry in summary.get("page_results", []) if entry["page"] != page_id]
