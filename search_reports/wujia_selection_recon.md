...
- reject CBETA-tainted or NC sources
...
- `嘉泰普燈錄` appears in the same Commons CADAL file-list page:
  - https://commons.wikimedia.org/wiki/Commons:Library_back_up_project/file_list/CADAL/03
  - The manifest exposes `CADAL02019619 嘉泰普燈錄（一）.djvu` through `CADAL02019622 嘉泰普燈錄（四）.djvu`.

Rights note:
- These are Commons-hosted scan families, so they are the right kind of lead for this repo.
- I did not promote them to acquired status because I did not download and validate the files in this pass.
- I also did not treat any CBETA / NC-derived exact-title hit as reusable.

Local follow-up:
- Source-family README created at `C:\woodblocks\J23nB134_Wujia_Selection_Open_Leads\README.md`
- Next step, if desired, is a download-and-validate pass on one of the three open scan families rather than the exact-title CBETA trail.
