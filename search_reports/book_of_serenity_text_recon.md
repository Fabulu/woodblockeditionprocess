...
1. `Book of Serenity / 萬松老人評唱天童覺和尚頌古從容庵錄`
...
- The root page shows the Wikisource site license footer and lists `卷001` through `卷006`.
...
- The root page lists `序` and `卷001` through `卷010`.
- The raw wikitext endpoint is reachable for the volume pages; I confirmed `卷005` and `卷001` return clean raw text.
- The repo already has a partial local text package at `C:\woodblocks\Blue_Cliff_Record_Wikisource_PD_old`, but it currently only contains `序` and `卷001` through `卷004`.
...
- The Book of Serenity scan set already exists locally in `C:\woodblocks\Book_of_Serenity_NLC_Commons`, but that is outside this free-text pass.
