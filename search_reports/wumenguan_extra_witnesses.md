# Wumenguan Extra Witness Search Report

Date: 2026-04-12

Scope: open/reusable Wumenguan witness leads only. No shared ledgers were edited.

## Priority leads processed

1. `WUL-bunko31_e1101`
- Source page: https://commons.wikimedia.org/wiki/File:WUL-bunko31_e1101_%E7%84%A1%E9%96%80%E9%96%A2.pdf
- Source URL: https://archive.wul.waseda.ac.jp/kosho/bunko31/bunko31_e1101/bunko31_e1101.pdf
- Local folder: `C:\woodblocks\Wumenguan_WUL-bunko31_e1101_Waseda_Commons`
- Local file: `WUL-bunko31_e1101_mumonkan.pdf`
- Rights basis: Commons says `Image: Public domain`; source is Waseda University Library.
- Result: downloaded and validated. PDF header checked and file size matched the Commons listing.

2. `WUL-bunko17_w0033`
- Source page: https://commons.wikimedia.org/wiki/File:WUL-bunko17_w0033_%E7%A6%85%E5%AE%97%E7%84%A1%E9%96%80%E9%96%A2.pdf
- Source URL: https://archive.wul.waseda.ac.jp/kosho/bunko17/bunko17_w0033/bunko17_w0033.pdf
- Local folder: `C:\woodblocks\Wumenguan_WUL-bunko17_w0033_Waseda_Commons`
- Local file: `WUL-bunko17_w0033_zenshu_mumonkan.pdf`
- Rights basis: Commons says `Image: Public domain`; source is Waseda University Library.
- Result: downloaded and validated. PDF header checked and file size matched the Commons listing.

## Broader search

- I ran a broader Commons sweep for additional open Wumenguan WUL-bunko leads.
- No new Wumenguan WUL-bunko witness leads surfaced in that sweep beyond the two priority items above and the already-known local Waseda copy.

## What succeeded

- Open-license basis was confirmed from the Commons file pages before download.
- Both priority witnesses were acquired locally.
- Both local PDFs were validated as PDF files and checked against the Commons-reported sizes.

## What remains pending

- Additional open Wumenguan witness searches beyond the WUL-bunko Commons leads.
- Any future acquisitions should continue to preserve the source-page URL and rights basis inside each witness folder README.
