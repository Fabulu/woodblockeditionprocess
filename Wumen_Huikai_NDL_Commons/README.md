# Wumen Huikai Record / NDL Commons

- Work: `無門慧開禪師語錄`
- Priority: medium
- Source page: https://commons.wikimedia.org/wiki/File:NDL2537788_%E7%84%A1%E9%96%80%E9%96%8B%E5%92%8C%E5%B0%9A%E8%AA%9E%E9%8C%B2.pdf
- Direct file URL: https://commons.wikimedia.org/wiki/Special:Redirect/file/NDL2537788_%E7%84%A1%E9%96%80%E9%96%8B%E5%92%8C%E5%B0%9A%E8%AA%9E%E9%8C%B2.pdf
- DOI: `10.11501/2537788`
- Rights basis: Commons file page is categorized `PD-Japan` and `PD-scan (PD-Japan)`. Commons file-page license text is the controlling rights statement.
- Local files:
  - `NDL2537788_wumen_kai_record.redownload.pdf`
  - `NDL2537788_wumen_kai_record.redownload2.pdf`
  - `source_page.html`
- Status: downloaded and validated with `pypdf` (`70` pages).
- Notes: use `NDL2537788_wumen_kai_record.redownload2.pdf` as the canonical local integrity anchor (`246,780,631` bytes; SHA-256 `782397C4D09627ECAF40EB6EBEF698E96B45A7FD743FC9A3FD6BB766D3319494`).
- Notes: the Commons file page also exposes the structured-data DOI and a `70`-page file summary; `source_page.html` captures both locally.
- Notes: earlier local copies in the same folder should not be treated as the canonical witness.
